...
class Editor:
    ...
    def assign(self, input):
        ...
        try:
            ...
            manuscriptRI = int(resultManuscript[0])
            cursor2.close()

            # check if manuscript RI and reviewer RI match
            query5 = "SELECT COUNT(*) FROM ReviewerInterests WHERE RICode = %d AND ReviewerId = %d;" % (manuscriptRI, reviewerId)
            cursor5 = self.con.cursor(buffered=True)
            cursor5.execute(query5)
            match = cursor5.fetchone()
            numResults = int(match[0])
            cursor5.close()
            ...
            print("Invalid. Usage: assign manuscriptid reviewerid")
    ...
    def reject(self, input):
        try:
            ...
            print("Invalid. Usage: reject manuscriptid")
    ...
    def accept(self, input):
        try:
            ...
            print("Invalid. Usage: accept manuscriptid")

    def typeset(self, input):
        try:
            manuscriptId = int(input[1])
            pp = int(input[2])
            ...
            print("Invalid. Usage: typset manuscriptid pp")
    ...
    def publish(self, input):
        try:
            ...
            print("Invalid. Usage: publish issueId")
